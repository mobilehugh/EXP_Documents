//attributes data for toy

:image_file: toy_materiel_spare_part.png
:image_folder: pre_rolls
:image_description: A piece of equipment with no obvious use
:image_artist: Dolly aimage. Prompt HM 
:image_date: 2024
:image_size: 1

:title_description: a piece of equipment with no obvious use
:title_prefix: This toy looks like

:toy_name: Spare Part for a Howitzer
:toy_department: Materiel
:toy_wate: nil
:toy_exps: 42
:toy_value: 42
:tech_level: 10
:toy_info: 1 spare part for a howitzer
:toy_xref: materiel.adoc#_spare_parts


// this checks nature of the include call
// if being used in a table on a persona record the toy_print_out is supplied

ifeval::["{toy_print_out}" != "false"]
include::pre_rolls:partial$format_toy.adoc[]
endif::[]

// put random stuff here
To find out more about a pharma analyer xref:hardware:artillery.adoc#_howizer[Howitzer, window=_blank]
