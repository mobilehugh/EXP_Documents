//attributes data for lazer bomb

:image_file: toy_biorepair_pharma_anal.adoc
:image_folder: pre_rolls
:image_description: A box with holes on the top and science on the sides.
:image_artist: Dolly aimage prompt HM
:image_date: 2024
:image_size: 1

:title_description: a box with holes on the top and science on the sides
:title_prefix: This toy looks like

:toy_name: Pharma Analyzer
:toy_department: biorepair
:toy_wate:  10 kg
:toy_exps: 250
:toy_value: 50000
:tech_level: 10
:toy_info: Identifies pharma. +42 on pharma related Tasks
:toy_xref: biorepair.adoc#_pharma_analyzer


// this checks nature of the include call
// if being used in a table on a persona record the toy_print_out is supplied

ifeval::["{toy_print_out}" != "false"]
include::pre_rolls:partial$format_toy.adoc[]
endif::[]