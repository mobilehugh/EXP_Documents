//attributes data for toy

// FIXME toy_materiel_spare_part.png redacted

:image_file: rp_aa_not_on_screen.svg
:image_folder: pre_rolls
:image_description: A piece of equipment with no obvious use
:image_artist: Dolly aimage. Prompt HM 
:image_date: 2024
:image_size: 1

:toy_description: a piece of equipment with no obvious use
:toy_description_prefix: This toy looks like

:toy_name: Spare Part of Howitzer
:toy_department: Materiel
:toy_wate: nil
:toy_exps: 42
:toy_value: 42
:tech_level: 10
:toy_info: 1 spare part for a howitzer
:hardware_xref: materiel.adoc#_spare_parts
:toy_xref: toy_materiel_.adoc#_spare_part_of_howitzer

// put random stuff here
// To find out more about howitzers xref:hardware:artillery.adoc#_howizer[Howitzer, window=_blank, rel="noopener noreferrer"]
