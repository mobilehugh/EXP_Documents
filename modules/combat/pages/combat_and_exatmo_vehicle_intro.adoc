...
Battling spacecraft could be tens of kilometres apart. 
...
Spacecraft must be traveling at almost the same speed to engage.
...
Research teams are located xref:uncombat:task_rolls.adoc#_research_team_composition[somewhere in this book,window=_blank, rel="noopener noreferrer"]. 
...
.*Combat Patches*
* Non Drive Patch (doors, hulls) Normal (42 + % goal)
* Inatmo Drive Patch Hard (84 + % goal)
* Exatmo Drive Patch Tough (126 + % goal)
* Special Drive Patch Impossible (168 + % goal)

For example, a mechanic attempts to repair the fusion drive by ten percentage points. 
This repair would be a exatmo drive patch with a difficulty of tough (126 + 10) task for the mechanic.
If she is successful, the fusion drive will move from 8% effectiveness (disabled) to 18% effectiveness.
The drive is still damage and once it is used to escape or ram it will likely fail again.

The mechanic could also repair an airlock that is only opening 40%.
The expedition needs it to open 60% to get the bomb off the spacecraft.
This would be a non-drive patch, normal (42 + 20) difficulty task.
The repair allows the expedition to jettison the ticking bomb. 
