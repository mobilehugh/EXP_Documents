...
The attribute that powers mental mutations is Mind.
...
The persona holding the object must win a PSTR roll vs the mutant's MND.
...
The mutant still has a Mind for her other mental mutations, vocations, or will power checks.
...
The player uses the PSTR wate allowance table but substitutes her persona's PSTR with her MND. 
...
The mutant's Mind is not doubled when attacking with mental mutations.
...
If an involuntary target is holding onto something, the referee may allow MND vs PSTR attribute challenge.
...
The xref:i-roll_playing_rules:CH03_AttributesDEX.adoc#_moverate[Movement Rate,window=_blank] table is consulted replacing dexterity with the mutant's Mind. 
...
If there is an advantage, the player can switch the persona's PSTR with her MND.
...
She can lift ten times her xref:i-roll_playing_rules:CH18_Encumbrance.adoc[Wate Allowance,window=_blank] using her MND instead of PSTR.
