...
// robotassembly  Anthros building robots to replace anthros

== Robot Type
Determining robot type is byzantine, meticulous and painful.
This is one reason that the https://github.com/misterquetoo/EXP_Game_Tools:[EXP_GAME_TOOLS] were developed.

Each attribute level has it's own column. 
The player follows down each column recording any robot IDs in that column.
...
For example, if the total number of Hs exceeded all other robot IDs the persona would be a hobbot.
...
=== ATTACKS 
determines the attacks that the robot can have.
...
=== DEFENCES 
determines the combat defences that the robot can have.
...
=== RANDOM 
determines how many random peripherals are attached to the robot.
...
=== ADAPTABILITY 
represents the adaptability of the robot.
...
=== VALUE 
is the standard currency value of the most basic version of the robot type.
The more peripherals attached to the bot, the more valuable it becomes.
This represents the simple base value.

=== SIZE 
indicates whether a robot is designed for outdoor use or indoor use.
...
however, robots cannot be made larger by choice.
...
// cable_face_desat Android migraines are the worst

== Fabricator Type

This is the family that the robot was designed and manufactured for.
Unless otherwise specified, a bot will not harm or allow to come to harm any member of its base family.
So, if a robot were to have an equine base family, then the persona would not be allowed to damage any equine that she met.
Nor would the robot allow the base family to accidentally come to harm.
Every robot type, except for androids and combat robots, must obey this tenet.

Non-lethal attacks that inflict small amounts of damage can be employed against a robots base family.
A robot will protect a member of its base family from other expedition members also.
...
Whenever the player wants to challenge the protection of her base family, she must make a control factor roll for her robot.
...
*7) Disregard Base Family:* This does not count as a roll on Robotic Attack table Four.
