...
.*Phenomic Mutations*
* <<_phenomic_mutations_list,Any Mutation>>
* <<_no_defects,No Defects (start up)>>
* <<_combat,Combat Mutations>>
* <<_uncombat,Uncombat Mutations>>
...
Psionic mutations like: ESP, gyrokinesis, and telekinesis can pass through any barrier if the mutant has proof that the target exists.
However, phenomic mutations are often limited by terrain, and the persona must be able to see or touch the target. 
Phenomic mutations which manipulate the environment, such as mental blast, or launchable quills, are subject to terrain effects.

==== Distance
The most common is the ranged mutation.
Ranged mutations can be directed at any target within the range of the mutant.
The range is determined by a specific attribute and the EXPS level of the attacker.
In the case of phenomic mutations there are a wide variety of attributes that can determine range: CON, DEX or STR.
For the author's convenience all examples below use Mind (MND). 
...
The growth may have a CHA penalty of minus 0 to 3 (d4-1) points, but no other penalties can apply.
If the adipomegaly is on the mutant's face, they will not suffer any AWE penalty or attack roll penalty.

NOTE:: This mutation was formerly called Fat Cell Accumulation
...
Attractemesis makes this mutant very interesting to a specific type of persona. 
...
Attractemesis works best on flora or fauna, but more intelligent personas can succumb as well.
...
Attractemesis should not result in a constant swarm of giant deadly fauna attacking the expedition.
...
NOTE:: Formerly called Attraction Odour
...
Buoyancy lets the mutant match the density of liquids or gases, allowing her to walk on them without breaking the meniscus and sinking.
...
Buoyancy does not allow the mutant to fly by walking on air. 
...
NOTE:: This mutation was formerly called Density Manipulation. 
...
The carapace is an unsightly build up of thick semi-pliable skin.
...
Dermatofortia has both a cosmetic and defensive effect on the mutant.
The cosmetic change is not a negative one. 
If the persona has a high charisma (CHA) the plates will look handsome and appropriate.
The structurally changed skin is simply the mutant's skin.
The skin structure change does not alter any attributes or affect the persona's ability to wear armour.
For example, skin structure changes have no restrictiveness or encumbrance penalties. 
The impact is purely on Defence Rating, and dermatofortia offers not other special abilities.
Dermatofortia can improve the mutant's defence (DEF) considerably.
So plated skin increases the persona's Defence Rating by 150.
If the mutant's defence was previously 500 her defence is now 650.

Often mutants with this mutation are averse to wearing armour out of pride or comfort

include::wetware:partial$phenomic_dermatofortia.adoc[]
...
On this mutant's belly is a 4 cm thick build up of nutritious and tasty regenerating flesh.
...
If you need a random xref:hardware:appearance.adoc#_colour_your_whirled[Colour,window=_blank, rel="noopener noreferrer"].
...
All organic personas within the area of effect must make a poison roll.
Targets that lose their poison roll suffer muscle weakness that paralyses them.
Targets that win their poison roll can crawl at one h/u.
Incapacitated targets are defenceless for 1 unit per level of intensity of the attack.
Intensity level 10 nausea poison would paralyse targets for ten units.
...
The duration of effect is determined by the xref:hardware:pharma.adoc[Pharma,window=_blank, rel="noopener noreferrer"]. 
...
Haste does not work up to normal encumbrance, and fails at burdened or lift only encumbrance.
...
The plume of smoke can have a strong xref:aliens:biology.adoc#_aroma[Aroma,window=_blank, rel="noopener noreferrer"] and distinct xref:hardware:appearance.adoc#_colour_your_whirled[Colour,window=_blank, rel="noopener noreferrer"].
...
Laryngoclasia allows the mutant to create noises so loud they are destructive. 
...
A 2nd level mutant has a radius of effect of 4 hexes.

include::wetware:partial$phenomic_laryngoclasia.adoc[]

.*4h Range Example*
* 0h to 1h = 4-32 (immediate)
* 1h to 4h = 3-24 (ranged)
* 5h = 2-16 (past)

The acoustic energy damages every target in the area of effect.
Fellow expedition members are also affected by the sonar attack.
...
Laryngoclasia can shut down organic and inorganic sensors.
...
. Determine insertion xref:hardware:a_introduction.adoc#_toy_type[Type,window=_blank, rel="noopener noreferrer"]
. Determine insertion xref:combat:hit_locations.adoc[Location,window=_blank, rel="noopener noreferrer"]
...
The chance of malfunction is determined by xref:uncombat:sphincter_rolls.adoc[Sphincter Roll,window=_blank, rel="noopener noreferrer"].
...
NOTE:: This mutation was formerly called Increased Metabolism.
...
The mouths are smaller, malformed, craggy toothed, fully functional mouths.
...
The organ is very sensitive, and will not aggravate internal injuries by pumping litres of blood into the persona's abdomen or skull.
...
For more information about damage and healing jump to xref:uncombat:health.adoc[Health,window=_blank, rel="noopener noreferrer"].
...
The player can choose the colour, or generate it at xref:hardware:appearance.adoc#_colour_your_whirled[Colour,window=_blank, rel="noopener noreferrer"].
...
The mutant also registers as a mild radiation anomaly.

NOTE:: This mutation was formerly called Phosphorescent Skin.
...
This mutant has light, hairless, almost translucent skin.
...
NOTE:: This mutation was formerly called Photosynthetic Skin.
...
The beam is her head's size and usually has a distinct xref:hardware:appearance.adoc#_colour_your_whirled[Colour,window=_blank, rel="noopener noreferrer"].
...
Or the player can let the dice decide by jumping to xref:hardware:appearance.adoc#_colour_your_whirled[Colour,window=_blank, rel="noopener noreferrer"].
...
If a fevered discussion brings the players to an impasse, they should consult the xref:uncombat:sphincter_rolls.adoc[Sphincter Roll,window=_blank, rel="noopener noreferrer"].
...
Scalomorphia allows her to decrease or increase her size.
...
This mutant is vulnerable to infections.
Her condition is not limited to microbial agents alone.
All manner of contagion, both real and imagined, afflicts the mutant.
Her Constitution will work normally against all other kinds of saves: poison, radiation, etc.

NOTE:: Formerly called No Resistance To Disease
...
The mutant has poison producing glands that can eject poisonous spittle some distance.
...
Intensity varies from 9-17 (1d8 + 9) intensity plus the mutant's experience level.
...
Pharma that relieves pain lasts half as long for this mutant.
...
NOTE:: This mutation was formerly called Double Physical Pain
...
Her Constitution works normally against illness, damage and injury, but offers resistance to poison rolls.
When making a poison related save the player cannot user person's CON.
Pharma effects and adverse effects are doubled.

NOTE:: This mutation was formerly called No Resistance To Poison
...
The mutation transmogriphosis comes with an additional layer of cosmetic musculature, flexible bones, and controllable connective tissue.
...
The transmogriphosis is gross to watch and quite noisy. 
...
NOTE:: This mutation was formerly called Body Structure Change
...
The vibration punch is a Strike attack that inflicts 3-12 (3d4) hit points, plus 1 hit point per Level.
...
If you want a raw roll (combat, uncombat and defects) xref:wetware:phenomics.adoc#_phenomic_mutations_list[jump here,window=_blank, rel="noopener noreferrer"].


=== No Defects
Best for start up phenomic mutations.
It is like a raw roll but has no defects.
