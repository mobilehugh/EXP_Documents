...
.*Psionic Mutations*
* <<_psionic_mutations_list,Any Psionic Mutation>>
* <<_no_defects,No Defects (start up)>>
* <<_combat,Combat Mutations>>
* <<_uncombat,Non Combat Mutations>>
* <<_defective,Defective Mutations>>
...
If a mutation has a duration of one unit per MND, then the mutant can make the mutation last 12 units if she has a 12 MND.
...
For combined attacks the players may need consult the xref:uncombat:sphincter_rolls.adoc[Sphincter Roll,window=_blank, rel="noopener noreferrer"].
...
Aisthoclasis breaks the senses of an organic target.
This mutation can turn off one of the target's senses.
The attack is only effective against organic personas.
The target must win a psionic roll or lose the described sensory input.
...
The more critical the sense, the more likely the target will win the psionic roll. 
Removing the sensation of pain can benefit the target in certain circumstances. 

include::wetware:partial$psionic_aisthoclasis.adoc[]

NOTE: This mutation was formerly known as Sensory Deprivation.
...
Repeated episodes of erasing information will raise a persona's sense of paranoia and suspicion.

Amnesiafication cannot be used to reverse engineer what the target persona knows. 
The mutant must know that a particular memory is present before erasing it. 
Amnesiafication cannot be used to fish for what a target knows by erasing random memories.

NOTE:: This mutation was formerly known as Information Eradication.
...
If a fight does break out between expedition members, it will always be xref:combat:non_lethal_combat.adoc[Non-Lethal Combat,window=_blank, rel="noopener noreferrer"].
...
Hyperfocus allows the mutant to the whole brain to obsess over one specific problem.
...
Hyperfocus should not short circuit session puzzles or a campaign arch.
...
Hyperfocus should advance the story, not short circuit the story.

Hyperfocus does not allow the mutant to predict the future or solve the mysteries of consciousness.
...
NOTE: This mutation was formerly known as Heightened Brain Talent.
...
Indensification allows the mutant to change the density of a target.
...
All targets must make a save vs psionic attack.
...
If a target repels the indensification, the mutant may try the next unit again.
...
NOTE:: This mutation was formerly called Density Control.
...
Infoporia allows the mutant to store and transfer information from one persona to another.
...
If the source is __not__ a willing transmitter of the knowledge, she gets to make a psionic save.
If the source wins the psionic roll, the knowledge is blocked from the mutant.
...
When infoporia has been blocked for the mutant, she must have one restful sleep before attempting transmission again.
...
Infoporia cannot be used to reverse engineer what the target persona knows. 
The mutant must know that a particular memory is present before copying it. 
Infoporia cannot be used to fish for what a target knows beyond the source knowledge.

NOTE:: This mutation was formerly called Knowledge Transmission. 
...
Levitation cannot overcome a failed gravity system in xref:combat:combat_and_exatmo_vehicle_intro.adoc[Spacecraft Combat,window=_blank, rel="noopener noreferrer"]. 
...
If there is discussion about whether an object has enough 'metal' to be attracted consult the xref:uncombat:sphincter_rolls.adoc[Sphincter Roll,window=_blank, rel="noopener noreferrer"].
...
If the mass mind doubles the range of necroprojection, does it affect the mutant?
If intense discussions arise remember that xref:uncombat:sphincter_rolls.adoc[Sphincter Roll,window=_blank, rel="noopener noreferrer"] can help answer the unanswerable. 
...
Mental blasts can be emitted without interrupting any other task.
...
Myokinesis allows the mutant to control a muscle group and its associated motor nerves.
...
Myokinesis is divided into peripheral and central muscles.
...
Subtle Myokinesiss do not get a psionic roll and are automatically successful. 
For example, dropping a piece of paper or turning a doorknob the wrong way would automatically succeed.
Dangerous Myokinesiss require the target to lose a psionic roll to work.
...
Myokinesiss like stopping the heart, breathing, and inducing cardiovascular collapse are lethal.
The target must win a psionic roll or die from the attack. 

Both central and peripheral Myokinesiss can be carried on dying or recently dead personas.
For example, a recently dead persona could posthumously raise its offensive middle talon. 
The mutant could keep the heart of a dead persona beating with Myokinesis.

NOTE: This mutation was formerly known as Muscle Manipulation.
...
Necroprojection instantly drains hit points from the target(s) caught in the area of effect.
...
Jump to xref:uncombat:health.adoc#_death[Death,window=_blank, rel="noopener noreferrer"] for more info about persona death.
...
Once the mutant regains consciousness, she may immediately use her necroprojection again.

The mutant cannot control the range of the necroprojection. 
...
NOTE:: This mutation was formerly called Death Field Generation.
...
Neuroemesis incapacitates organic targets with nausea.
This mutation works on all organic personas, including anthros and aliens.
Robots are not affected by a neuroemesis.
Each persona in the area of effect must win a psionic roll, or become violently ill and are incapacitated.
Those who win the psionic roll are still nauseated and but only partially incapacitated.
Partially incapacitated includes: -500 on attack rolls, half movement rate (no sprinting or bolting), -42 on all task rolls, and halved attributes for attribute rolls.
These effects immediately stop when the persona is out of the area of effect.

Neuroemesis is a useful mutation for immobilizing everything within the area of effect.
However, the nausea attack is not discerning, and every organic persona within range is affected.
Both friends, and foes, are hit with the nausea effect.
With experience, the mutant can exclude specific personas from being affected by their mutation. 
The mutant can protect one target per 2 EXPS levels from their neuroemesis mutation.
For example, a 4th level mutant could keep 2 of her expedition members from being affected by the repulsion field.

NOTE: This mutation was formerly known as Nausea Field Generation
...
The attack puts the target into a sensory anaesthetic state., 
...
Phasemorphia allows the mutant to convert her body into a homogeneous solid, liquid or gas.
...
If an active discussion erupts the players can consult the xref:uncombat:sphincter_rolls.adoc[Sphincter Roll,window=_blank, rel="noopener noreferrer"].
...
A liquid mutant can drown a defenceless target faster than regular water. 
...
The horrendous specifics of suffocation are covered under xref:uncombat:terrains.adoc#_atmosphere[Terrain,window=_blank, rel="noopener noreferrer"].

NOTE: This mutation was formerly known as Molecular Phase Transformation
...
Phasoclasia allows the mutant to change targets into a solid, liquid or gas.
Phasoclasia cannot be used by the mutant to transmute herself.
...
If the target wins a psionic roll, she is unaffected by the psionic attack.
...
Unlike <<_phasemorphia,Phasemorphia>>, the target cannot function when transmuted.
...
A more detailed read on total transmutation is available <<_phasemorphia,here>>.
...
NOTE: This was formerly called Molecular Phase Transmutation.
...
If the campaign uses the attribute social standing, this mutant cannot have an SOC less than 900.

NOTE: This was formerly called Sociability Field. 
...
Physiostasis is used to reduce malevolent effects on the body. 
...
A mutant with Physiostasis is unaffected by infections or cancers.
...
NOTE: This mutation was formerly called Mental Control Over Physical State.
...
If an active discussion arises regarding whether, or not, something is metallic consult the xref:uncombat:sphincter_rolls.adoc[Sphincter Roll,window=_blank, rel="noopener noreferrer"]
...
For example, mental control, Myokinesis directly attack the mutant.
...
Psychopenia does not mean the mutant is lacking Intelligence, bravery or emotional weakness.
...
However the mutant is not affected by her own sonoclasia
...
Below is the radius of effect for a second level mutant.

include::wetware:partial$psionic_sonoclasia.adoc[]

.*4h Range Example*
* 0h to 1h = 3-24 (immediate)
* 1h to 4h = 2-16 (ranged)
* 5h = 1-8 (past)
...
The mutant can protect one target per EXPS Level from being damaged within the radius of effect.
So a 2nd level mutant could keep two of her team mates from being blasted by sonoclasia.
...
The xref:anthros:movement_rate.adoc[Movement Rate,window=_blank, rel="noopener noreferrer"] table is consulted replacing Dexterity with the mutant's Mind. 
...
Regular xref:uncombat:terrains.adoc#_falling[Falling,window=_blank, rel="noopener noreferrer"] damage applies.
...
When throwing random objects at targets the most complicated ruleset to use is xref:combat:combat_and_robots.adoc#_ramming[Robotic Ramming,window=_blank, rel="noopener noreferrer"].
...
The arm can lift an amount determined using the mutant's MND for wate allowance (1/3 WA using MND).
...
Using the xref:uncombat:encumbrance.adoc#_encumbrance_levels[Encumbrance,window=_blank, rel="noopener noreferrer"] rules the mutant must have light encumbrance.
...
The mutant uses her MND instead of her DEX on the xref:anthros:movement_rate.adoc[Movement Rate,window=_blank, rel="noopener noreferrer"] table.
...
For example, the mutant may start to exhibit the whiny attitude of the targeted persona. 
...
She can lift ten times her xref:uncombat:encumbrance.adoc[Wate Allowance,window=_blank, rel="noopener noreferrer"] using her MND instead of STR.
...
If you want a raw roll (combat, uncombat and defects) xref:wetware:psionics.adoc#_psionic_mutations_list[jump here,window=_blank, rel="noopener noreferrer"].
