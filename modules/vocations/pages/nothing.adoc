...
These challenge rolls could be used to get to the front of a line or get a larger portion of gruel.
...
The higher her social standing, the more influential the nothing persona.
...
For more information about SOC, jump to xref:attributes:social_standing.adoc[social standing,window=_blank, rel="noopener noreferrer"].
...
=== Worth
Nothings calculate monetary worth completely differently from other vocations.
...
The treasure system complicates and convolutes treasure generation.
...
If you want more information about treasure jump over to  xref:hardware:treasure.adoc[Treasure,window=_blank, rel="noopener noreferrer"].

include::vocations:partial$nothing_worth.adoc[]

NOTE: A nothing cannot get an artifact when calculating net worth.

include::vocations:partial$nothing_worth_type.adoc[]
...
== Gifts 
Gifts are nothing tasks that are automatically completed. 
No task roll is required.

include::vocations:partial$nothing_gifts.adoc[]
...
However, the player should state the vocation aspiration when creating the persona.
...
This could be some cash, a credit card or a vacation souvenir.
...
=== Compendium 
A non-exhaustive list of nothing tasks arranged by increasing difficulty.
...
Recall Name (24)::
    Recall a referee persona's name. 
...
Tie a Tie (42)::
    Correctly adorn a tie on herself.

Queue Jumping (42)::
    Get herself past the queue.
...
Tie Another's Tie (84)::
    Correctly adorn a tie on another persona.

Queue Trashing (84)::
    Get the expedition past the queue.
...
=== List
An alphabetical list of the tasks described above.
