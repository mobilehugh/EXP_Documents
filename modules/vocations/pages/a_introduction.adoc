...
:description: Space suit wearing humanoid carrying briefcase and thumb up for a ride in outer space. 
...
The information and tasks that the persona can utilize are self-taught.
...
NOTE: *Previously vocations were called classes.
...
Each vocation represents a natural aptitude that includes sparse disorganized bits of information.
As the persona increases her experience level in her vocation, she improves her skill set.

.*Vocation Types*
* xref:vocations:biologist.adoc[Biologist,window=_blank, rel="noopener noreferrer"]: Studies and observes organic things
* xref:vocations:knite.adoc[Knite,window=_blank, rel="noopener noreferrer"]: Does orderly or chaotic things
* xref:vocations:mechanic.adoc[Mechanic,window=_blank, rel="noopener noreferrer"]: Repairs and improves inorganic things
* xref:vocations:mercenary.adoc[Mercenary,window=_blank, rel="noopener noreferrer"]: Breaks organic and inorganic things
* xref:vocations:nomad.adoc[Nomad,window=_blank, rel="noopener noreferrer"]: Survives using few things
* xref:vocations:nothing.adoc[Nothing,window=_blank, rel="noopener noreferrer"]: Infatuated with material things
* xref:vocations:spie.adoc[Spie,window=_blank, rel="noopener noreferrer"]: Deceiver and breaker of things
* xref:vocations:veterinarian.adoc[Veterinarian,window=_blank, rel="noopener noreferrer"]: Repairs and improves organic things
...
However, some robots can function as a vocation, and some alien species can have vocations in their society.

The vocations are composed of knowledge and tasks. 
Vocation knowledge is what the persona knows. 
Knowledge is used by the persona to succeed at tasks. 
Each vocation has its own set of knowledge.
Each vocation has its own set of tasks.
Some tasks are limited to a solitary vocation, others can be used by all vocations. 
...
Veterinarians need Charisma, and nomads need Awareness and mercenaries need Hit Points.
...
A persona with a 4 INT and a 14 AWE can become a biologist.
...
include::vocations:partial$vocation_attributes.adoc[]
...
include::vocations:partial$vocation_selection_example.adoc[]

== Knowledge
A vocation's knowledge represents things that the persona knows. 
The knowledge base of the vocation represents it's literal definition.
Knowledge is split into two parts: interests and expertise. 
Each vocation has tables devoted to knowledge this would inform that vocation.
A veterinarian has knowledge around healing, and a mercenary has knowledge around combat.
This does not mean that there are some overlapping knowledge.

Knowledge is not limited to book knowledge.
Some of the knowledge is definitely about how to do things. 
Recalling details about alien anatomy is knowledge, and being skilled at cutting meat cakes is also knowledge.
Knowing how to sharpen a knife is knowledge, but actually sharpening the knife is a task.
Without getting over philosophical: anything that assists a player's roll in succeeding at a task is knowledge.

There are two areas of knowledge: interest and expertise. 
Each vocation has dedicated interest and expertise tables.
The player can roll randomly on, or select from, these tables.
Players can also create bespoke knowledge, but this is only recommended for experienced play tables. 

=== Interests
Interests are broad areas of knowledge.
For example, research is an interest.
Typical areas of interest are research, repair, and combat, etc.
Interests are wide in scope by design and can be used to assist all kinds of tasks. 

A biologist may be interested in taxonomy (which will help identify aliens), or culture (which will help them understand a culture).
...
=== Expertise
Expertise is more specific knowledge. 
They are narrow in scope.
An expertise usually has a concise description like: pistols, cloning, tracking. 
Expertise knowledge can only use be for specific tasks. 
However, this knowledge is much more helpful when making task roles. 
A persona with a combat interest can use that knowledge in any combat situation.
If the persona had pistol expertise she could only use that knowledge when the task is pistol related.
...
=== Gifts
Gifts are tasks that the persona has committed to knowledge.
The persona can complete gifted task with the player having to win a task roll.
For example, a veterinarian has the gift of Quick Fix. 
This allows her to immediately heal her colleague a small amount. 
A mercenary has the gift of ambidextrous combat with swords. 
This allows her to use two swords without making the task roll. 
As the persona advances in her vocation she can add more gifts. 

The player can create a bespoke task as a gift.
If the desired gift does not exist as a vocation task the player table and referee must vet the gift.
The existing task lists for the vocation should be used to guide de novo gifts. 

NOTE:: Alternate names for gifts have been removed. Below is for flavour and fun.

Each vocation has it's own silly name for their gifts. 
If a player refers to her persona's task roll as a gift that should be understood. 
It is also possible for a player to create a bespoke 

include::vocations:partial$vocation_gifts.adoc[]

== Tasks
A task is anything that a persona tries to do.
Vocations have lists of tasks that are best performed by a specific vocation.
However, vocations can attempt each other's tasks as task rolls. 
Some tasks are entirely vocation specific.
For example, assassination can only be attempted by a spie, and a mercenary would not be good at tracking.

NOTE: A task can be skill, a procedure, a concept, an action. 

The persona's interests, expertise and experience level determine how well the persona performs at a task.
Initially some tasks will be out of reach as the player could not beat the listed difficulty.
For the intricate details of how to resolve tasks in EXP jump to xref:uncombat:task_rolls.adoc[Task Rolls].

=== Compendium
Each vocation has a compendium of tasks that are appropriate for the vocation. 
The compendium is organized by task difficulty. 
The explanations of tasks in the compendium is limited on purpose.

It is expected that the player will record some of the key tasks from the compendium on the persona record. 
The compendium is not exhaustive and it is expected that the table will have tasks that are not listed.

=== List
The task list is organized by vocation.
It is a list of tasks in alphabetical order.
In all other respects it should have the same info as the compendium.
...
Villainous veterinarians and altruistic anti-knites may see their tasks weaken.
Nomads living in decadent luxury will find their survival tasks starting to rust.
...
A murderous veterinarian may find that her tasks don't work well on those who know the truth.
...
For example, a mercenary persona may add in the mechanic vocation to ensure maintenance of the the spaceship's naval artillery.  
...
The persona advances both levels simultaneously and can use tasks from both vocations.
...
The persona's mercenary tasks freeze at her exit level.
She begins collecting experience points as a first-level biologist with zero EXPS.
She can still use her mercenary tasks but only collects experience as a biologist.
...
== Robots
The vocations in this section represent the anthropomorphic pursuits of knowledge and skill.
...
To pursue a player persona robot, jump to xref:robots:a_introduction.adoc[Robots].

== Aliens
The vocations in this section are anthropomorphic pursuits of knowledge and skill.
Aliens do not pursue a specific vocation, and are considered a vocation unto themselves.
Aliens have goals of energy procurement and reproduction and have evolved to fulfill those goals.
...
Aliens can earn experience, gain levels, and improve their evolutations and combat skills.
...
To pursue a player persona alien, jump to xref:aliens:a_introduction.adoc[Aliens].
