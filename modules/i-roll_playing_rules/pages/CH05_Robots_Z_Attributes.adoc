...
A robot's physical strength of ten is the same as an anthro's physical strength of ten.
...
The regular attributes are awareness (AWE), charisma (CHA) and social standing (SS).
...
For more info about this attribute test jump to xref:i-roll_playing_rules:CH05_Robots_Z_CF.adoc[Control Factor,window=_blank].

The second irregular attribute is mental strength (MSTR).
Robot personas have a mental strength score of zero.
A zero mental strength does not indicate that robots are psionically vulnerable and easily manipulated.
Robots do not have mental strength because their consciousness does not exist within the Think Space.
...
If a robot must test her MSTR, she will use her intelligence score instead. 
...
The prime attributes are constitution, dexterity, intelligence and physical strength.
...
include::ROOT:partial$CH05_Table_Robotic_Attribute_Levels.adoc[]

A player that rolls a three for PSTR prime has a PSTR prime of 3.
...
Her persona's regular attributes scores are 11 AWE, 8 CHA and 653 SS.
She records these scores on her persona record.

The player then generates her prime attributes (CON, DEX, INT and PSTR) using a 1d4.
Her persona's prime attributes are 1 CON, 2 DEX, 3 INT and 4 MSTR. 
She then generates the regular scores for each of the prime attributes.
Based on her prime attributes, she rolls 1d8 for her CON, 1d6+7 for her DEX, 1d6+13 for her INT and 1d10+19 for her PSTR.
...
Her PSTR has a prime of 4 and a score of 27.
