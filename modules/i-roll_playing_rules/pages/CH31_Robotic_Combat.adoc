...
:description: Drawing of martial arts trained humanoid robot throwing two humans at once.
...
:date: 1990
:license: CC BY-SA 4.0
:description: Drawing of angry janitorial robot waving broom at human who messed up the floor.
...
include:i-roll_playing_rules:CH05_Robots_Z_CF.adoc[tag=Control_Factor]
...
If a janitorial robot was used to clean a dirty bathroom.
...
If the robot is chasing a target running away the target's speed is subtracted from the robot's speed.
...
If the robot and the target are heading towards each other the target's speed is added to the robot's speed.
...
If a 40 kg robot were to ram a 120 kg target for 24 HPS of damage the robot would inflict one third damage.
...
An attacker could damage a robot's sensors, or power plant, without ever aiming for that particular part.
