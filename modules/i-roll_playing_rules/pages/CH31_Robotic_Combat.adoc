...
:description: Drawing of martial arts skilled humanoid robot throwing two humans at once.
...
.. xref:i-roll_playing_rules:CH05_Robots_Z_CF.adoc[Control Factor,window=_blank] rolls may be required.
. Robots may have a fabricator type that they protect.
. Bonus Proficient is only for baked-in bespoke weapons.
.. Bonus Non-Proficient is used for misused peripherals. 
. Ramming is different from ramming skill.
.. Ramming attack is required to use the Bonus Proficient.
...
For a robot to intentionally harm a persona from her fabricator type would require a xref:i-roll_playing_rules:CH05_Robots_Z_CF.adoc[Control Factor,window=_blank] Roll.
...
:date: 1990
:license: CC BY-SA 4.0
:description: Drawing of angry janitorial robot waving a broom at a human who messed up the floor.
...
Ramming is a Type A attack on the robot's combat table.
...
If the player's attack roll is higher than the target's armour rating, the target gets rammed.
The player can add her persona's Type A bonus to her attack roll.
The player can only add her Type A  Bonus Non-Proficient to her ramming attack roll.
...
Robots cannot use the Type A damage adjustment for ramming. 
