...
* the attack roll is a specialized challenge roll versus an armour rating with random effect (damage).
** 1d1000 vs AR
...
Attack Roll:: 
Attack roll is a combat term.
It indicates that an attacker is trying to damage her opponent. 
The higher the attack roll the more likely to damage the target.
An attack roll that is higher than the target's armour rating is called a hit.
An attack roll that is less than the target's armour rating is called a miss.


Attack Type A:: 
Attack Type A are non-powered thrusting and striking attacks. 
Examples are swords, clubs, daggers, axes, claws, fists, and hooves.

Attack Type B:: 
Attack Type B are non-powered projectile attacks. 
These attacks are ranged but are not mechanically powered. 
Examples are bows, rocks, darts, bolos, shooting quills, or spitting acid.

Attack Type C:: 
Attack Type C are powered attacks of any sort. 
