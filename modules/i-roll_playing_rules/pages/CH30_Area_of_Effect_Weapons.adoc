...
Her persona's combat table modifies the attack roll.
How the Type AOE weapon is delivered determines which bonuses improve her attack roll.


Type B AOE is non-powered projectile attacks that the persona throws at the target hex (hand grenade).
Type C AOE is a powered projectile that the persona shoots at the target hex (grenade launcher, or aerosol).
Type A AOE would be a suicide vest or land mine.
