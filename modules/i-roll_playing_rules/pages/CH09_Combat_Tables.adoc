...
EXP initially developed the combat table for anthro player personas.
Robots and aliens came along later.
The combat table language aligns most efficiently with anthro personas.
The persona's attributes make up the first component of her bonus proficient.
The persona's experience level and vocation make up the second component of her bonus proficient.
The persona's BP determines her BNP and MR on the combat table. 
...
Let's walk through the calculation of a combat table for the following persona:
...
The robot combat table is only for offensive systems baked-in during fabrication.
The player uses the BP for baked-in weapons and the BNP for malfunctioning peripherals.

Robots do not use artifact weapons.
An android with a vocation will use an artifact weapon as an anthro with that vocation.
A robot can end up with two combat tables: one for artifact weapons and one for her built-in attacks.

A combot with a baked-in laser pistol would add the Bonus Proficient to her attack roll.
A janitorial robot with a cleaner that malfunctions as a laser pistol would use her BNP.
The BNP is zero.
...
However, in the next attack, the player could roll 954, an almost certain hit.

include::ROOT:partial$CH09_Table_Robot_CT.adoc[]

=== Robot Combat Table Example
A 4th level janitorial robot has the following attributes: AWE 12 CHA 11 CON 12 DEX 10 INT 12 MSTR 0 PSTR 16.
She three offensive systems.
Her baked-in pistol would use her BP for type C attacks.
Her ramming skill would use her BP for type A attacks.
Her acid squirted from a misused cleaner would use her BNP.
...
Aliens do not have a maximum roll.
An alien with naturally evolved lasers shooting tentacles would use her BP for Type C attacks. 
...
If the alien has a vocation, she will use the anthro vocation-based combat table for her artifact combat table.
An alien can have two combat tables: one for artifact weapons and one for her natural attacks. 

include::ROOT:partial$CH09_Table_Alien_CT.adoc[]

=== Alien Combat Table Example
An 8th level alien has the following attributes: AWE 12 CHA 11 CON 12 DEX 10 INT 12 MSTR 15 PSTR 16.
Her naturally evolved energy wave would use her BP for Type C attacks.
Her ferocious tentacle slap would use her BP for Type A attacks.
