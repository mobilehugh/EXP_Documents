...
This is the data that a referee needs to run a referee persona.
...
Most of the data generated here is for the Roll Playing Rules.
Elements like armour rating, wate allowance, movement rate are calculated here.
Most groups that have opted for the Role Playing Rules will not need this data.
There are some elements that spill over into the Roll Playing Rules.
For example, anthro type, sub type, mutations, EXPS level and vocation serve both rule systems.
With experience the referee and players will hone what info they need from both systems. 
...
If the referee prefers a young or elder anthro it is recommended to select that here.
...
// use a tag include from the updated combat table
