...
CF is an attribute unique to the family Machinidae.
...
.. Combination of INT, INT range and EXPS Level
. <<_cf_roll_difficulty,CF Roll Difficulty>>
. Determine Target Roll
.. Target Roll = CF Roll Difficulty
...
== Make the Roll
The player must roll higher than or equal to the target roll for the Control Factor Roll.
The roll is made on a 1d20 and the persona's control factor (CF) is added to the roll.
The CF Roll is a specialized Attribute Roll that applies only to robot personas.

== Loss of Control
A failed CF Roll will indicate that the player has temporarily lost control of her persona.
This could be a player or referee robot persona that has reverted to their production state.
The player still runs the robot persona, but she can only do things that the robot would do

On occasions a CF Roll failure has a heavy combot reverting to wanting to kill the expedition.
CF fails should be fun. 
