...
* <<_checks,Checks>> (Look for certain numbers)
...
Attack rolls have a check, previously called a critical check. 
...
If the player rolls a __natural__ 42 the referee can have the weapon malfunction.
...
NOTE: *A check is also called a Critical Roll or Critical Check.*
...
Previously called a critical check or critical roll.
If the player rolls a __natural__ 20 she rolls another 1d20 and adds it to the task roll.
All critical checks are detailed in the section xref:i-roll_playing_rules:CH16_Special_Rolls_Critical.adoc[Critical Checks,window=_blank].
...
The referee may call for a PSTR attribute roll. 
...
There are no checks for challenge rolls. 
However, it is easy enough to include them if they make things more fun.
