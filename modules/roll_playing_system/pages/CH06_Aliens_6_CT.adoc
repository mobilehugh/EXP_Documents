...
For more info jump to xref:roll_playing_system:CH09_Combat_Tables.adoc[Combat Tables,window=_blank].
...
The player must create a combat table for her tool using alien persona using the xref:roll_playing_system:CH04_Anthros_CT.adoc[Anthro Combat Table,window=_blank].
If the alien has a vocation, the player will generate another combat table using the alien's vocation type.
If the alien does not have a vocation, the player uses the nothing vocation on the xref:roll_playing_system:CH04_Anthros_CT.adoc[Anthro Combat Table,window=_blank].
