...
The tactical combat system in the Roll Playing System uses an attack matrix called the attack table.
...
The player must create a attack table for her tool using alien persona using the xref:roll_playing_system:CH04_Anthros_CT.adoc[Anthro Attack Table,window=_blank].
If the alien has a vocation, the player will generate another attack table using the alien's vocation type.
If the alien does not have a vocation, the player uses the nothing vocation on the xref:roll_playing_system:CH04_Anthros_CT.adoc[Anthro Attack Table,window=_blank].
