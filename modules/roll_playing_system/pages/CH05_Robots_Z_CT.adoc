...
The tool using robot fabricates a attack table from xref:roll_playing_system:CH04_Anthros_CT.adoc[Anthro Attack Table,window=_blank].
If the robot has a vocation (like an android), the player will generate the attack table using the alien's vocation type.
If the robot does not have a vocation, the player uses the nothing vocation on the xref:roll_playing_system:CH04_Anthros_CT.adoc[Anthro Attack Table,window=_blank].
