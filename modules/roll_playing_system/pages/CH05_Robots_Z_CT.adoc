...
The robot attack table is easier to generate and manage than the anthro attack table.
There are fewer calculations on the robot attack table.

The robot attack table is for attacks baked-in during fabrication.
A robot's attack table need only include the robot's baked-in attack types.
However, calculating the complete attack table to future proof the robot persona.
...
== Robots and Weapons
This attack table is for the robot's baked-in attacks generated during robot fabrication.
Most baked-in attacks come from the robot's offensive systems section.
The robot attack table is not for external artifact weapons that the robot has picked up to use.
In most cases, robots will not be able to use weapons like an anthro persona.
If the robot has a vocation or can use weapons like guns and swords, the player must generate an additional attack table.
The tool using robot fabricates a attack table from xref:anthros:attack_table[Anthro Attack Table,window=_blank].
If the robot has a vocation (like an android), the player will generate the attack table using the alien's vocation type.
If the robot does not have a vocation, the player uses the nothing vocation on the xref:anthros:attack_table[Anthro Attack Table,window=_blank].
