...
Regardless of the special drive type, the vessel travels through xref:appendices:generic_mythos.adoc#_the_spaces[hyperspace,window=_blank, rel="noopener noreferrer"].
...
For more information about fuel and energy jump to xref:appendices:generic_mythos.adoc#_energy_and_power[Energy and Power,window=_blank, rel="noopener noreferrer"].
...
When a persona can use the ship's computer for a task roll, she enjoys a bonus of +4 per computer level.
For instance, a persona who can use the etiquette programme to help cater a fancy dinner.
If the computer level is 5, the persona gets a +20 bonus when making the meal. 
...
=== Steward 
Guest laden spacecraft require a ship's steward.
...
xref:combat:combat_and_exatmo_vehicle_intro.adoc[Spacecraft Combat,window=_blank, rel="noopener noreferrer"].
...
To create the airlock gun jump to xref:hardware:guns.adoc#_generate_type[Guns,window=_blank, rel="noopener noreferrer"]
...
For more information about vac suits jump xref:hardware:armour.adoc#_vac_suit[Vac Suits,window=_blank, rel="noopener noreferrer"].
