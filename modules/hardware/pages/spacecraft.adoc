...
Life support, gravity, Defence Rating, hull and shield (if present) are all part of the <<_defensive_onion,Life Support Onion>>.
...
They include airlocks, cold storage, workspaces, cabins, and corridors.
There is no system for generating these accessory systems.
What the 

Spacecraft size determines what can be attached to or built into the spacecraft. 
More specifically how much hull space is left after drives, computer and fuel have been taken into account.
...
NOTE: Airlocks do not deduct cargo space but require cargo space.
