...
A spacecraft is any vehicle can function outside planetary atmospheres and sail off into the wild black yonder.
...
=== Fuel Level
The drive types the spacecraft has, the more fuel the spacecraft may have.
...
A spacecraft requires a computer and crew to function.
...
=== Defence Rating 
The Defence Rating indicates the strength of the spacecraft's hull.
...
Other spacecraft features occupy potential cargo space.
These are cold storage, workspaces, cabins, and corridors.
All of these cargo space deductions are essential for an operational spacecraft.
The player must account for all the following items as she sees fit.
Plotting out the cargo and operational essentials has the most significant impact on spacecraft design. 
It is during this process that the players create a simple map of the spacecraft. 
...
A spacecraft without an airlock depends on inatmo boarding either planetside or in a larger spacecraft hanger.
...
The cargo airlock can move cargo or without depressurizing the spacecraft.
A cargo airlock requires at least 500 tonnes of vacant cargo space.
The cargo airlock also occupies cargo space.
A spacecraft loses 1 tonne of cargo space per 2 tonnes of cargo airlock size.
...
Spacecraft will have automated emergency equipment and processes.
...
If the spacecraft has a doctor, there will automatically be a sickbay.
...
Woah, it takes a lot of cargo to refit a bit spacecraft.
