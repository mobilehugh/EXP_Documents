...
// FIXME powered armour puppy cuteness


Biomech is a specialized form of armour that is servo-aided, computer-enhanced and intimately integrated with the wearer.
Typically biomech armour is sealed, carries its power plant and fully integrates with its accessories.
Biomech armour enhances the wearer's attributes by electronically amplifying the persona's natural abilities.
Biomech armour becomes an anatomical extension of the persona, fully integrating her with the armour and its artifacts.

NOTE: *Biomech was formerly Powered Armour.*

== Suit Fabrication
Every biomech suit is uniquely appointed with it's own accessories.
Kind of like lethal snow flakes. 

.Biomech Fabrication Checklist
. <<_core_suit, Core Suit>>
. <<_suit_accessories,Core Accessories Check>>
. <<_helmet,Helmet Accessories Check>>
. <<_arms,Arm Accessories Check>>
. <<_body_accessories,Body Accessories Check>>
. <<_legs,Leg Accessories Check>>
. *Have fun!*
...
The wearer can earn other attribute bonuses as additional accessories improve the armour.
Attribute increases are only in effect when the biomech armour is being worn and operational.

=== Strength Bonus
.STR + 4-10 (2d4+2)
****
persona STR + 4-10 = 12 + (2d4+2) = 12 + 8 = 20 STR in suit
****

=== Dexterity Bonus
.DEX + 1-6 (1d6)
****
persona DEX + 1-6 = 1d6 + 12 = 12 + 4 = 16 DEX in suit
****

.*Suit Accessory List*
* Air-conditioned (automatic)
* Armour hardened (automatic)
* 0-3 (1d4 - 1) additional core accessories
...
include::hardware:partial$biomech_accessories_core.adoc[]

Air Conditioned::
Every suit comes with air conditioning.
Waltz into lethal mechanized combat within the confines of luxuriously air-conditioned biomech armour.
Air conditioning is more like the life support of a spacecraft than a comfort tool.
Air Con also keeps the wearer from overheating due to the armour's internal heat generation.
Additional air conditioning units layer over each other improving comfort and safety.
...
Anti-Personnel::
Anti-personnel prevents costly suits of biomech armour from being overwhelmed by hordes of expendable fodder.
...
Anything holding onto the biomech armour must make a save versus mental attack or let go of the suit.
...
.CHA + 1-3 (1d3)
****
Wearer CHA =  1d4 + 12 = 12 + 2 = 14
****

Armour Hardened::
Hardening is simply an improvement of the toughness of the suit's alloy-ceramic hull.
Each armour hardening increases the suit's Defence Rating by 1d100.
...
Internal System::
An internalized system is less like an accessory and more like an entirely different role-playing game.
The internal system takes all the attributes of the biomech armour and surgically inserts them within the persona's body.
...
Internalizing a suit of biomech armour can only work if the referee permits this within her milieu and story.
...
The persona's jump distance is five times further than her usual xref:combat:move_in_combat.adoc#_jumping[Jump,window=_blank, rel="noopener noreferrer"]. 
The player uses her persona's armour-adjusted STR to calculate the extended jump.
...
Neural Synched::
With neural synchronization, the suit's servo system links directly to the wearer's cerebellum.
...
Biologic Hardened::
Biologic hardening protects the wearer from uncombat environmental conditions.
...
This biomech armour has a mini airlock that allows small items to transfer into the biomech armour's protective environment.

Zero-G Maneuvers::
Zog maneuver allows the persona to move about as if normal in the wateless three-dimensional terrain of zero gravity.

== Helmet
Helmet accessories are baked-in parts of biomech armour's helmet.
...
.*Helmet Accessories List*
* One communication (automatic)
* One visual aid (automatic)
* 0-3 (1d4 - 1) additional helmet accessories

=== Helmet Accessories

include::hardware:partial$biomech_accessories_helmet.adoc[]

Communication:: 
Helmet communications have a range of 10 kilometres and can only with other biomech armour radios.
The wearer cannot use helmet communications to jam signals or conduct any electronic countermeasures.
Multiple communication systems represent redundancy and range extension.
If this is a random accessory jump to xref:hardware:devices.adoc#_communicators[Communicators,window=_blank, rel="noopener noreferrer"].

Aerosol::
An aerosol attack can be spray from the biomech armour's helmet.
There will be 1-12 (1d12) aerosol charges available.
To determine the aerosol type jump to xref:hardware:aerosols.adoc#_generate_type[Aerosols,window=_blank, rel="noopener noreferrer"].

Grenade::
The grenade can launch from the helmet of the biomech armour.
Launched grenades are a  Shoot  attack, with a range three times the grenade's thrown range.
There will be 1-6 (1d6) grenades launchable from the helmet.
To determine the grenade type jump to xref:hardware:grenades.adoc#_grenade_type[Grenades,window=_blank, rel="noopener noreferrer"].
...
To determine the *pistol* type jump to xref:hardware:guns.adoc#_generate_type[Guns,window=_blank, rel="noopener noreferrer"].
...
This unique biomech armour feature is usually in suits of armour used by commanders.
...
.CHA + 1-6 (1d6)
****
persona CHA + 1-6 = 1d6 + 9 = 9 + 5 = 14 CHA when in suit
****
...
[discrete]
=== Biorepair Constitution Bonus
.CON + 1-3 (1d3)
****
persona CON + 1-3 = 1d3 + 15 = 15 + 3 = 18 CON when in suit
****

Device::  
Miscellaneous equipment is the most diverse random accessory found in the biomech armour's helmet.
The only limitation is that the miscellaneous artifact is less than 10 kgs in wate.
To determine the right device or gadget jump to xref:hardware:devices.adoc#_generate_type[Device,window=_blank, rel="noopener noreferrer"].

Scope:: 
The helmet has an integrated computer scope.
If the biomech armour has a helmet pistol, the scope will is automatically for that weapon.
To determine the type of scope jump over to xref:hardware:devices.adoc#_scopes[Scopes,window=_blank, rel="noopener noreferrer"].

Visual Aid:: 
Vizauds (visual-auditory enhancers) are bespoke to biomech armour.
...
Each vizaud that the biomech armour has installed increases the persona's AWE by 1-3 points.
If this vizaud is a random accessory, jump to xref:hardware:devices.adoc#_visual_aids[Visual Aids,window=_blank, rel="noopener noreferrer"].

[discrete]
=== Visual Aid Awareness Bonus
.AWE + 1-3 (1d3)
****
persona AWE + 1-3 = 1d3 + 15 = 15 + 3 = 18 AWE when in suit
****
...
Accessory points can be left unfilled during biomech armour generation.

.*Arm Accessory List*
* 1-3 accessory points per arm
* 10% chance of accessory point being filled

=== Arm Accessories

include::hardware:partial$biomech_accessories_arms.adoc[]
...
Generate the random detector by jumping to xref:hardware:devices.adoc#_detectors[Detectors,window=_blank, rel="noopener noreferrer"].

Aerosol::
There will be 1-12 (1d12) aerosol charges sprayable from biomech armour's limb.
To determine the aerosol type jump to xref:hardware:aerosols.adoc#_generate_type[Aerosols,window=_blank, rel="noopener noreferrer"].

Grenade::
There will be 1-6 (1d6) grenades launchable from the limb.
Launched grenades attack as Shoot weapons, with the range five times that of a thrown grenade.
To determine the grenade type jump to xref:hardware:grenades.adoc#_grenade_type[Grenades,window=_blank, rel="noopener noreferrer"].

Gun::  
This gun fires from the biomech armour's arm.
...
Device or Gadget:: 
Devices and gadgets represent most diverse accessory found in the biomech armour's arm.
...
To determine the device equipment jump to xref:hardware:devices.adoc#_generate_type[Devices,window=_blank, rel="noopener noreferrer"].

Accessory Point::
The suit of biomech armour has a bonus additional accessory point in its arm.
...
Biorepair:: 
This biomech armour has a built-in veterinary artifact built into its arm.
Veterinarian equipment is rolled in xref:hardware:biorepair.adoc#_veterinary_equipment_type[Biorepair,window=_blank, rel="noopener noreferrer"].
...
== Body
Body accessories are part of the body portion of the armour.
A body computer may have its access via a keyboard on the chest.
The biomech armour body includes one ventral (front) and dorsal (back) surface.

A medium-sized anthro persona cannot have more than four accessory points on either the front or back of the biomech armour body.
Extremely large personas, like an alien, may have more than two body surfaces. 
For example, there could be right and left flanks in addition to dorsal and ventral surfaces.

.*Body Accessory List*
* 2 storage compartments 
* 2-8 accessory points
* 15% chance of accessory point being filled

=== Body Accessories

include::hardware:partial$biomech_accessories_body.adoc[]

Artillery::
This mechano-trooper has an artillery piece built into the body of her biomech armour.
An artillery piece occupies four accessory points and can be the only accessory on either the ventral or dorsal surface.
If the body has less than four accessory points, the artillery piece will be the only accessory on the body. 
No suit of biomech armour can support more than one artillery piece.
Firing the artillery cannot harm the wearer of the biomech armour. 
To generate the artillery piece jump to xref:hardware:artillery.adoc#_artillery_type[Artillery,window=_blank, rel="noopener noreferrer"].

Bomb::
This mechano-trooper has an artillery piece built into the body of her biomech armour.
No suit of biomech armour can support more than one bomb.
There is a 50% chance that a generic missile will deliver the bomb with a 100 km range.
If there is no missile to launch the bomb, it is a self-destruct for the biomech armour.
If the bomb does not have a missile, the persona gets a 1-3 (1d3) increase to their Mind. To generate the bomb type jump to xref:hardware:bombs.adoc#_bomb_type[Bombs].

[discrete]
=== Walking Bomb Mind Bonus
.MND + 1-3 (1d3)
****
Persona MND + 1-3 = 1d3 + 8 = 8 + 2 = 10 MND when in suit
****

Communication::
The communicator in the body is a comprehensive comms device.
The body communicator has a range of 500 kilometres, encrypted channels, and three additional particular use channels.
...
A suit of biomech armour can only have one comprehensive comms device.
For further communication accessories jump to xref:hardware:devices.adoc#_communicators[Communicators,window=_blank, rel="noopener noreferrer"].

Computer Equipment::
Computer equipment is built into and powered by the biomech armour. 
...
To generate the computer equipment jump to xref:hardware:devices.adoc#_computers[Computers,window=_blank, rel="noopener noreferrer"].

Computer System::
The computer system integrates fully with the biomech armour wearer.
...
.INT + 1-4 (1d4)
****
persona INT + 1-4 = 1d4 + 16 = 16 + 2 = 18 INT when wearing suit
****

Gun::  
There is a rifle built into the biomech armour's body.
The rifle can be baked into the body or on an articulated rifle mount.
...
Energy weapons have an unlimited power supply from the armour's power plant.
To determine the rifle type jump to xref:hardware:guns.adoc#_generate_type[Guns,window=_blank, rel="noopener noreferrer"].

Biomedical System::
The body medical system is a medical accessory that prioritizes
the cardio respiratory organs of the wearer.
...
Device:: 
Miscellaneous equipment is the most diverse accessory found on the biomech armour.
The only limitation is that the miscellaneous artifact is not disposable.
There is no wate restriction on devices in the body of biomech armour.
To determine the device jump to xref:hardware:devices.adoc#_generate_type[Devices,window=_blank, rel="noopener noreferrer"].

Accessory Point::
The suit of biomech armour has a bonus additional accessory point on its body.
...
xref:uncombat:encumbrance.adoc[Encumbrance,window=_blank, rel="noopener noreferrer"]

Biorepair Equipment:: 
This biomech armour has a built-in veterinary artifact built into its body.
Veterinarian equipment is rolled in xref:hardware:biorepair.adoc#_veterinary_equipment_type[Vet Equipment,window=_blank, rel="noopener noreferrer"].
...
* 2 storage compartments
* 20% chance of an accessory point being filled

=== Leg Accessories

include::hardware:partial$biomech_accessories_legs.adoc[]
...
Aerosol::
There will be 1-12 (1d12) aerosol charges sprayable from biomech armour's leg.
To determine the aerosol type jump to xref:hardware:aerosols.adoc#_generate_type[Aerosols,window=_blank, rel="noopener noreferrer"].

Grenade::
There will be 1-6 (1d6) grenades launchable from the leg.
Launched grenades attack as Shoot weapons, with the range seven times that of a thrown grenade.
Often players will have the grenade launch be a kicking action.
To determine the grenade type jump to xref:hardware:grenades.adoc#_grenade_type[Grenades,window=_blank, rel="noopener noreferrer"].

Gun::  
This gun fires from the biomech armour's leg.
If the gun has ammunition, it will have five times the regular magazine capacity. 
Energy weapons have an unlimited power supply from the armour's power plant.
To determine the gun type jump to xref:hardware:guns.adoc#_generate_type[Guns,window=_blank, rel="noopener noreferrer"].

Motorized::
This leg accessory allows the suit of biomech armour to move at incredible speed by diverting resources to the legs.
The persona can only move when the motorized movement is activated.
A suit of biomech armour can only have one motorized movement system installed.
Motorized movement allows the exoskeleton to run away at triple xref:anthros:movement_rate.adoc[Movement Rate,window=_blank, rel="noopener noreferrer"] rate and not less than 15 hexes per unit.

Accessory Point::
The suit of biomech armour has a bonus additional accessory point in its leg.
The person could fill the empty accessory point with a future artifact.
The accessory point is not a pocket, and nothing can be stored within it.
...
If the biomech armour has a leg gun, the scope will is automatically for that weapon.
To determine the type of scope jump over to xref:hardware:devices.adoc#_scopes[Scopes,window=_blank, rel="noopener noreferrer"].
...
== Suiting Up
Biomech armour should fit into the referee's milieu and story.
...
The biomech armour may be patiently waiting for someone to step into its savage but convenient auto dresser.
The biomech armour may require multiple life-threatening neurosurgical interventions spread over numerous missions.

Undressing biomech armour is a difficult task as well.
...
Some biomech armour accessory prevents the persona from ever wanting to leave their combat cocoon.

When fully powered, the exoskeleton has a restrictiveness of one (unrestricted).
When powered down, the wearer is trapped in a tight-fitting coffin.
...
=== Aliens
A 6-tonne alien suited in biomech armour would be a formidable opponent indeed.
...
Alien species that are at the tool use level of creator can have biomech armour.
The shape of the biomech armour would match the shape of the alien inside.
If the alien shape indicates, they may have more than two arms to armour up.
If the alien is gigantic, they may have two flanks and ventral and dorsal body parts.

=== Robots
The biomech armour would be an outer layer of the robot.
...
A robot may shed its biomech armour layer like any other persona.

=== Anthros
Strangely shaped mutants are extremely unlikely to find biomech armour that fits them.
Mutants are unique anomalies, unlike aliens, which are part of an entire species.

== Combat
Combat is made more complex by biomech armour. 
...
This multitasking only applies to combat weapons built into the biomech armour. 
Multitasking does not apply to vocation skills, vet equipment, detectors, or specialized comms. 

== Power Source 
Biomech armour will not suddenly run out of power.
There will be lots of indicators and indications that the suit is running low.
Having personas with biomech armour constantly seeking power can be a challenging limitation to the powerful artifact.
Biomech armour has a power plant similar to that of robots.
...
== Suit Value
The base value of a suit of biomech armour is 10000000 eps (ten million).
...
The value of these accessory artifacts increases the value of the biomech armour.
The value of armour-specific accessories also increases the value of the biomech armour.
The value of these accessories is listed below.

include::hardware:partial$biomech_suit_value.adoc[]
...
:date: 1958
:license: CC BY-SA 4.0
:description: Rat inside robot head with controls and levers.
include::ROOT:partial$articulate.adoc[]
