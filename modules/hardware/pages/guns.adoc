...
To modify a toys' description jump to xref:hardware:appearance.adoc[Appearances,window=_blank, rel="noopener noreferrer"].
...
For all the deets jump to xref:combat:attack_types.adoc[Attack Types,window=_blank, rel="noopener noreferrer"].
...
==== Value 
The value indicates the MSRP (Money Suggested in role-playing) of the gun in eps.
...
Jump to xref:hardware:appearance.adoc#_colour_your_whirled[Colour] if you want a fancy beam colour.
...
.*Pharma*
Pharma is a special ammo type that injects a pharma into the target. 
In this case, the needler is a long range hypodermic needle.
To determine the med injected jump to xref:hardware:pharma.adoc#_pharma_type[Pharmaceuticals].
...
The value of the pharma is multiplied ten fold in this format.
Powerful pharma are limited to one dose only.
Note that a miss with this weapon has no effect and wastes the pharma.
...
Jump to xref:hardware:appearance.adoc#_colour_your_whirled[Colour].
...
The beam harmlessly passes through robots, walls, Argillian rock women, and armour.
...
:date: 2016-2021
:license: Copyright JEShields Stock Art. jeshields.com
:description: Punk rocker humanoid in skirt holding an enormous futuristic rifle.
include::ROOT:partial$articulate.adoc[]
