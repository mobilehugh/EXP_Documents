...
=== Parameters
For details on the special elements of grenade combat jump to xref:combat:area_attacks.adoc#_grenades[GrenadeCombat,window=_blank, rel="noopener noreferrer"].
...
To avoid being stunned, the player must win a xref:anthros:generate_hit_points.adoc#_system_shock[Damage System Shock,window=_blank, rel="noopener noreferrer"] roll (anthros only).
To avoid being toppled, the player must win a dexterity xref:uncombat:attribute_rolls.adoc[Attribute Roll,window=_blank, rel="noopener noreferrer"].
