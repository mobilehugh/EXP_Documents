...
For more detailed information about hit points, jump to xref:attributes:hit_points.adoc[Hit Points,window=_blank, rel="noopener noreferrer"].
...
For more details about anthro HPS, DSS and REC, jump to xref:anthros:generate_hit_points.adoc[Anthro HPS,window=_blank, rel="noopener noreferrer"].
...
To learn more about this convoluted and drawn-out process jump to xref:robots:damage_and_demolition.adoc[mechanical decay, window=_blank, rel="noopener noreferrer"].

== Healing
Healing is any process or task that returns HPS to the persona.
...
Combating age related attribute deterioration is a bizarre difficulty task roll.
...
Rest cannot repair a decreased INT or STR attribute.
Rest cannot reverse ageing.
Combating ageing and attribute deterioration are complicated veterinarian skills.
...
Jump to process of xref:robots:damage_and_demolition.adoc[mechanical decay, window=_blank, rel="noopener noreferrer"] to get the rundown on robots running down.
...
All organic personas progress through five age categories: child, youth, adult, elder, and aged.
...
Most anthros start age is a youth or young adult.
...
This starting age will usually put them in the late youth or young adult age range.
...
=== Youth
A youth is suffering through the age of transformation.
This transformation represents reproductive Awareness, independence and identity.
Most organic personas start in the youth age range. 
...
Attribute age category adjustments do apply to personas that age into youth.
The persona may be allowed to pursue a new vocation with these attribute changes. 

A person ascending into youth would suffer/enjoy the following attribute adjustments: AWE +1, CHA -5, CON +2, DEX -3, INT -1, MND -2, STR +2, and HPS +20%.
In general, youths are stronger, healthier and more irritating than their younger brethren.

When descending into youth from adulthood, only the negative adjustments apply.
...
Veterinarians are best at performing this stabilization, having specific skills to do so.
A successfully stabilized persona will cease to lose HPS at the lethal rate of 1 per minute.
If a stabilization task roll fails, the vet can immediately try again, but the anthro loses 1 HPS.
