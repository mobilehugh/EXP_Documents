...
We all react hysterically to a kilodie roll of 000, but it is no more likely than a roll of 604.
...
== Natural Roll
A critical check must arise from a natural roll.
No modifiers can cause a player to generate a critical check.
If the persona's roll and bonuses add up to 1000, this does __not count__ as a critical check.
...
TIP: Some tables allow the player to max the DMG roll. So a 1d8 weapon would do 8 HPS DMG.

NOTE: *Referees do not use critical checks against player personas.*

=== 1d100
The deci-die only has critical checks when making a task roll.
...
== Common Critical Check Table

include::uncombat:partial$critical_checks.adoc[]