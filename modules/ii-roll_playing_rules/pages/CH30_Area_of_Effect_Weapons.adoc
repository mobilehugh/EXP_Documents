...
=== Grenade Activation
There are many different ways to activate a grenade (pin, twist cap, etc.).
...
=== Range of Grenades
The target hex chosen by the player can be anywhere within throwing range of her persona.
...
=== Grenade Malfunction
The chance of a grenade being a dud  depends on whether it is an artifact grenade or a homemade mundane grenade.
Grenade malfunctions are determined by the unadjusted kilo die roll of the player, and bonuses attack are not considered.
Mundane area of effect weapons  have a high chance of malfunctioning or even exploding in the mitts of the persona.
Homemade grenades will detonate when thrown if the kilo die attack roll is less than 42.
They will not explode at all if the attack roll is less than 200.
Artifact grenades are proper military devices and are much more robust.
A referee may check for a malfunction if the kilo die attack roll is exactly 42.
The referee and player would use a sphincter dice challenge from  to determine the outcome of the malfunction.
...
== Attack Roll 
The standard attack roll on kilo-dice determines how successfully the persona has thrown the grenade.
...
A successful attack roll will indicate that the grenade has detonated in the target hex, having whatever effect that is described.
...
=== Attack Bonuses
Personas throwing grenades make an attack with a Type B weapon against a specific hex on the playing table (or against a general location if not using a grid system).
...
:width: 250px
:snark: Roll to see where it rolls.
:artist: HM
:date: 2010
:license: CC BY-SA 4.0
...
=== Attack Roll
An attack with an aerosol requires no roll attack, and targets in the area of effect are automatically effected.
...
The important details about aerosols and their ranges, effects, and peculiarities are described in 
...
=== Stream Area of Effect
This is the most common aerosol area of effect, and it is hallmark of a military aerosol.
...
Spherical area of effect aerosols fire a charge out to their maximum range, and then explode outwards, with a spherical area of effect.
...
=== Aerosol Confinement
When aerosols are used in close quarters some of the worst gaming complications can arise.
