...
For non-combat cutting, the lazer cutter works as expected.
...
The player must win a physical strength roll against the initial damage, or her persona loses control of the dagger.
...
Then ten additional hit points plus the persona's Damage Adjustment are added to the total damage.
...
For help with amputation location jump to with xref:i-roll_playing_rules:CH36_Hit_Locations.adoc[Hit Locations,window=_blank].
...
The player does not include her damage adjustment when determining the chance of an amputation attack.

==== Kirlian Focus Tool
A solid lite sabre is the preferred xref:i-roll_playing_rules:CH08_Vocations_Knite.adoc[Knite's,window=_blank]  weapon.
Those lite sabres that act as a xref:iii-hardware:CH48_Misc_Equip.adoc#_kirlian_talisman[Kirlian Talisman,window=_blank] for Knites are ancient and of limited number. 
...
The target will take 1-4 (1d4) hit points of damage and a psionic attack of intensity 1-10 (1d10) plus the firer's mental strength.
...
The target hex has an armour rating of 600, and a miss uses the grenade miss table.
The typical grenade area of effect carnage follows.

If the player targets a single unlucky persona, she must win an attack roll against the target's armour rating.
...
To determine the grenade type jump to xref:iii-hardware:CH45_Grenades.adoc#_grenade_type[Grenade Type,window=_blank].
...
The statue has no armour rating and is defenceless against chisels or hammers.
...
To place the garotte is a Type A attack that requires a successful ambush.
...
Safe removal of the toss garotte requires a vet or mechanic performance roll.
