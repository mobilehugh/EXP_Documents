...
To avoid being stunned, the player must win a xref:i-roll_playing_rules:CH04_Anthros_HPS.adoc#_damage_system_shock[Damage System Shock,window=_blank] roll (anthros only).
To avoid being toppled, the player must win a dexterity xref:i-roll_playing_rules:CH16_Special_Rolls_Attributes.adoc[Attribute Roll,window=_blank].
...
TheThe player need only have the mini grenade land in the target hex. 
...
The size of the damaged area of effect is rolled on the Disruption Event Table. 
...
The radiation grenade is also known as a Neutron grenade, or a Microgravity.
...
== Grenade Parameters
For details on the special elements of grenade combat jump to xref:i-roll_playing_rules:CH30_Area_of_Effect_Weapons.adoc#_grenades[GrenadeCombat,window=_blank].

=== Range 
The range of a grenade depends entirely on the thrower.
A persona can throw a grenade one hex per point of physical strength
