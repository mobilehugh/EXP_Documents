...
Defensive peripherals are anything that makes the robot harder to hit, damage or control.
...
Unlike the convoluted attack tables, there is only one Defensive Peripherals Table.
Each robot type has a clear direction on how to approach this table. 
Some robots have no defensive peripherals.
Others are designated with multiple peripherals.  
...
The player generates the armour type from the xref:hardware:armour.adoc[Armour,window=_blank, rel="noopener noreferrer"] section in hardware.
...
The bomb task is assigned a difficulty, and the robot gets to work. 
...
Only light and heavy offensive combots (types C-L and C-H) can use their offensive peripherals while engaging in evasive action.
...
To generate the mutation jump to the specific table xref:wetware:psionics.adoc#_non_combat_mutations[Non Combat Psionic Mutations,window=_blank, rel="noopener noreferrer"].
