...
These peripherals give the robot the tools to perform its tasks.
...
Every robot gets one roll on the Basic Peripherals Table. 
...
These creative impasses can be bridged using the decision aid xref:uncombat:sphincter_rolls.adoc[Sphincter Rolls,window=_blank, rel="noopener noreferrer"].

== Basic Peripherals
Basic peripherals are not fancy. 
A basic peripheral will only mildly alter the robot's function or value.

include::robots:partial$peripherals_basic_list.adoc[]

== Fancy Peripherals
Fancy peripherals can be game-changers.
Mild-mannered robots can pursue a vocation.
Most fancy peripherals represent a combination of hardware and software.
These peripherals are not overly detailed as they are uncombat peripherals.
An uncombat peripheral does not mean that it is not powerful.
